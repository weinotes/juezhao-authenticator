...
describe('TOTP', () => {
  ...
  describe('parseOtpauthUri', () => {
    it('解析标准URI', () => {
      const r = parseOtpauthUri('otpauth://totp/Google:user@gmail.com?secret=JBSWY3DPEHPK3PXP&issuer=Google');
  ...
  describe('validateSecret', () => {
    it('有效密钥', () => { expect(validateSecret('JBSWY3DPEHPK3PXP')).toBe(true); });
    it('空密钥', () => { expect(validateSecret('')).toBe(false); });
    it('短密钥', () => { expect(validateSecret('JBSWY3D')).toBe(false); });
  });

  describe('generateSecret', () => {
    it('生成Base32', () => { expect(generateSecret()).toMatch(/^[A-Z2-7]+$/); });
